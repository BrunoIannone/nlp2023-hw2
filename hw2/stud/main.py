...
for hyperparameter in tqdm.tqdm(hyp_comb):
    if tuple(hyperparameter) == (0.01, 0.001, 0.5, 0.5, 0, 0.01) or tuple(hyperparameter) == (0.01, 0.001, 0.5, 0.5, 0, 0) or tuple(hyperparameter) == (0.01, 0.001, 0.5, 0.5, 0, 0.001):
        print("Skipped successfully")
        continue
    print(hyperparameter)
    lin_lr = hyperparameter[0]
    lstm_lr = hyperparameter[1]
    dropout = hyperparameter[2]
    lin_dropout = hyperparameter[3]
    lin_wd = hyperparameter[4]
    lstm_wd = hyperparameter[5]
    ...
    model = glove.Glove_WSD(utils.EMBEDDING_DIM, utils.HIDDEN_DIM,
                            50265, len(vocab.labels_to_idx), utils.LAYERS_NUM, None,vocab.idx_to_labels,lin_lr,lstm_lr,dropout, lin_dropout,lin_wd,lstm_wd)
    #model = mod.WSD(utilz.LANGUAGE_MODEL_NAME,len(vocab.labels_to_idx.keys()),vocab.idx_to_labels,fine_tune_lm=True)
    model = model.load_from_checkpoint(os.path.join(utilz.DIRECTORY_NAME, '../../model/0.001, 0.002, 0.01.ckpt'),map_location='cpu')
    ...
    logger = TensorBoardLogger(os.path.join(utilz.DIRECTORY_NAME,"tb_logs/glove_lr_try") + str(lin_lr) + ", " + str(lstm_lr) + ", " + str(dropout)+ ", " + str(lin_dropout)+ ", " + str(lin_wd) + ", " + str(lstm_wd)+ ", " + str(utils.DROPOUT_EMBED))
    ...
    trainer = pl.Trainer(log_every_n_steps=21,max_epochs = utilz.NUM_EPOCHS,callbacks=[EarlyStopping(monitor="val_loss", patience=5,mode='min'), ModelCheckpoint(filename= str(lin_lr) + ", " + str(lstm_lr) + ", " + str(dropout)+ ", " + str(lin_dropout)+ ", " + str(lin_wd) + ", " + str(lstm_wd)+ ", " + str(utils.DROPOUT_EMBED) ,monitor='valid_f1',save_top_k=1,every_n_epochs=1,mode='max',save_weights_only=False,verbose=True,dirpath=os.path.join(utilz.DIRECTORY_NAME,'../../model/glove_lr_try'))],logger=logger,accelerator='gpu')#,auto_lr_find=True, auto_scale_batch_size=False)
